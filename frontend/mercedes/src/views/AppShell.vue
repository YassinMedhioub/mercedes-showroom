...
function pickLandingPage() {
  ...
  if (saved && pageComponents[saved]) {
    // If trying to land directly on car details, ensure we have an id
    if (saved === 'car') {
      const savedId = localStorage.getItem('selectedCarId')
      if (savedId) selectedCarId.value = savedId
      else {
        currentPage.value = 'cars'
        localStorage.setItem('currentPage', 'cars')
        return
      }
    }
  ...
  if (e.key === 'currentPage') {
    ...
    if (cp && pageComponents[cp]) {
      if (cp === 'car') {
        ...
        if (sid) selectedCarId.value = sid
        else return // ignore invalid car landing
      }
      currentPage.value = cp
    }
  }
  if (e.key === 'selectedCarId') {
    selectedCarId.value = e.newValue
  }
}
...
function go(page, extras = {}) {
  // Block access to app pages when logged out
  if (!isLoggedIn.value && page !== 'login') {
    currentPage.value = 'login'
    return
  }
  ...
  if (page === 'car') {
    ...
    // Leaving car page → clear stale id
    if (currentPage.value === 'car') {
      selectedCarId.value = null
      localStorage.removeItem('selectedCarId')
    }
  }
