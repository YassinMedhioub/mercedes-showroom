      ...
      >
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex items-center gap-2 text-xs"
               :class="isDarkMode ? 'text-zinc-300' : 'text-zinc-600'">
            <span>{{ language === 'FR' ? 'Faites glisser pour tourner' : 'Drag to rotate' }}</span>
            <button @click="toggleSpin" class="px-2 py-1 rounded border"
              :class="isDarkMode ? 'border-zinc-700 hover:bg-zinc-800' : 'border-zinc-300 hover:bg-zinc-100'">
              {{ spinning ? (language === 'FR' ? 'Pause' : 'Pause') : (language === 'FR' ? 'Lecture' : 'Play') }}
            </button>
          </div>
        </div>
        ...
        >
          ...
          <div
            ...
            :class="isDarkMode ? 'text-zinc-300' : 'text-zinc-600'">
            {{ language === 'FR' ? 'Aucune image 360°' : 'No 360° frames' }}
          </div>
        </div>
      </section>
      ...
      >
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1">
          <button
            ...
            :key="idx"
            class="shrink-0 w-24 h-16 rounded-lg overflow-hidden border transition"
            :class="[
              activeIndex === idx
                ? (isDarkMode ? 'border-white' : 'border-zinc-800')
    ...
    >
      <div class="flex items-center justify-between mb-2">
        <h2 class="text-lg font-semibold">
          {{ language === 'FR' ? 'À propos du modèle' : 'About this model' }}
        </h2>
        <button
          type="button"
      ...
      <div v-if="aiLoading" class="space-y-2">
        <div class="h-4 rounded animate-pulse" :class="isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'"></div>
        <div class="h-4 rounded animate-pulse" :class="isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'"></div>
        <div class="h-4 rounded w-2/3 animate-pulse" :class="isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'"></div>
      </div>

      <!-- Error -->
      <p v-else-if="aiError" class="leading-relaxed"
         :class="isDarkMode ? 'text-zinc-300' : 'text-zinc-700'">
        {{ aiError }}
      </p>

      <!-- AI text -->
      <p v-else-if="aiDesc" class="leading-relaxed"
         :class="isDarkMode ? 'text-zinc-300' : 'text-zinc-700'">
        {{ aiDesc }}
      </p>
...
function goBack() {
  emit('navigate', { page: 'dashboard', sub: 'cars' })
}
...
const descCacheKey = computed(() => `ai-desc:${language.value}:${carName.value}`)
  ...
  try {
    const res = await fetch('/api/ai/describe-car', {
      ...
      body: JSON.stringify({
        ...
        language: language.value === 'FR' ? 'fr' : 'en'
      })
    })
    ...
    aiError.value = language.value === 'FR'
...
watch([car, language], () => fetchAiDescription(), { immediate: true })
</script>
